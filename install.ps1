...
function Install-ContextMenu {
    ...
    Write-Success 'Menu contextual del Explorador instalado.'
}
...
$soundEnabled = Read-YesNo '¿Reproducir sonido al iniciar la terminal?' $true
...
if (Test-Path $assetsSource) { Copy-Item (Join-Path $assetsSource '*') $assetsTarget -Recurse -Force }

$startupSound = Join-Path $assetsTarget 'aparicion.mp3'
if ($soundEnabled -and -not (Test-Path $startupSound)) {
    Write-WarnMessage 'No se encontro assets\aparicion.mp3. El inicio continuara sin sonido personalizado.'
}
...
foreach ($command in 'Start-PPWorkSession','Get-PPStatus','Export-PPSession','Open-PPLatest','Open-PPExportFolder','Stop-PPSession','Show-PPPanel','Set-PPSessionVariable','Get-PPSessionVariable','Remove-PPSessionVariable','Set-PPLocation') {
...
Write-Host ''
Write-Host 'Coloca el archivo aparicion.mp3 en la carpeta assets del proyecto antes de instalar.' -ForegroundColor DarkGray
Write-Host ''
