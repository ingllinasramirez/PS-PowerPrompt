...
function Read-YesNo {
    ...
    if ([string]::IsNullOrWhiteSpace($value)) { return $Default }
    $value.Trim().ToLowerInvariant() -in @('s','si','sí','y','yes')
}

function Remove-ProfileBootstrap {
    param([Parameter(Mandatory)][string]$ProfilePath)
    if (-not (Test-Path -LiteralPath $ProfilePath)) { return }
    $start='# >>> PS-PowerPrompt >>>'; $end='# <<< PS-PowerPrompt <<<'
    $content=Get-Content -LiteralPath $ProfilePath -Raw -ErrorAction SilentlyContinue
    if ($null -eq $content) { return }
    $cleaned=[regex]::Replace($content,'(?s)'+[regex]::Escape($start)+'.*?'+[regex]::Escape($end),'').TrimEnd()
    if ($cleaned -ne $content) {
        Copy-Item -LiteralPath $ProfilePath -Destination "$ProfilePath.backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')" -Force
...
function Install-ProfileBootstrap {
    ...
    $content=[regex]::Replace($content,'(?s)'+[regex]::Escape($start)+'.*?'+[regex]::Escape($end),'').TrimEnd()
    Copy-Item -LiteralPath $ProfilePath -Destination "$ProfilePath.backup-$(Get-Date -Format 'yyyyMMdd-HHmmss')" -Force
...
function Test-OllamaApi {
    try { Invoke-RestMethod -Uri 'http://127.0.0.1:11434/api/tags' -Method Get -TimeoutSec 4 | Out-Null; return $true } catch { return $false }
}

function Ensure-OllamaReady {
    Write-Step 'Preparando Ollama para asistencia local...'
    $ollama=Get-OllamaExecutable
    if (-not $ollama) {
        $winget=Get-Command winget -ErrorAction SilentlyContinue
        if (-not $winget) { throw 'No se encontro winget. Instala App Installer de Microsoft Store y ejecuta nuevamente el instalador.' }
        ...
        if ($LASTEXITCODE -ne 0) { throw "winget no pudo instalar Ollama. Codigo: $LASTEXITCODE" }
        $machinePath=[Environment]::GetEnvironmentVariable('Path','Machine')
        $userPath=[Environment]::GetEnvironmentVariable('Path','User')
        $env:Path="$machinePath;$userPath"
        $ollama=Get-OllamaExecutable
        if (-not $ollama) { throw 'Ollama fue instalado, pero no se encontro ollama.exe. Reinicia Windows y ejecuta nuevamente el instalador.' }
        Write-Success "Ollama instalado: $ollama"
    } else { Write-Success "Ollama detectado: $ollama" }

    if (-not (Test-OllamaApi)) {
        Write-Step 'Iniciando servicio local de Ollama...'
        Start-Process -FilePath $ollama -ArgumentList 'serve' -WindowStyle Hidden | Out-Null
        $ready=$false
        foreach($attempt in 1..30) { Start-Sleep -Seconds 1; if(Test-OllamaApi){$ready=$true;break} }
        if (-not $ready) { throw 'Ollama no inicio su API local en http://127.0.0.1:11434.' }
    }
    Write-Success 'API local de Ollama disponible.'

    $models=& $ollama list 2>$null | Out-String
    if ($models -notmatch [regex]::Escape($OllamaModel)) {
        Write-Step "Descargando modelo $OllamaModel. Esta operacion puede tardar varios minutos..."
        & $ollama pull $OllamaModel
        if ($LASTEXITCODE -ne 0) { throw "No fue posible descargar el modelo $OllamaModel." }
    }
    Write-Success "Modelo disponible: $OllamaModel"

    $body=@{model=$OllamaModel;stream=$false;messages=@(@{role='user';content='Responde solamente: OK'})} | ConvertTo-Json -Depth 6
    $response=Invoke-RestMethod -Uri $OllamaEndpoint -Method Post -ContentType 'application/json' -Body $body -TimeoutSec 300
    if ([string]::IsNullOrWhiteSpace([string]$response.message.content)) { throw 'Ollama no respondio la consulta de validacion.' }
    Write-Success 'Ollama respondio correctamente desde PowerShell.'
}
...
$moduleSource=Join-Path $projectRoot 'modules\PSPowerPrompt'; $moduleTarget=Join-Path $installRoot 'modules\PSPowerPrompt'
$uiSource=Join-Path $projectRoot 'ui'; $uiTarget=Join-Path $installRoot 'ui'
$scriptsSource=Join-Path $projectRoot 'scripts'; $scriptsTarget=Join-Path $installRoot 'scripts'
$assetsSource=Join-Path $projectRoot 'assets'; $assetsTarget=Join-Path $installRoot 'assets'
...
Write-Host ''; Write-Host "PS-PowerPrompt Installer $InstallerVersion" -ForegroundColor White
Write-Host 'Presiona Enter para aceptar cualquier valor predeterminado.' -ForegroundColor DarkGray; Write-Host ''
...
$aiEnabled=Read-YesNo '¿Instalar y configurar la asistencia inteligente local con Ollama?' $true

Write-Step 'Validando archivos del proyecto...'
foreach($required in @($moduleSource,$uiSource,$scriptsSource)){if(-not(Test-Path -LiteralPath $required)){throw "No se encontro: $required"}}
$sourceSound=Join-Path $assetsSource 'aparicion.mp3'
if($soundEnabled -and -not(Test-Path -LiteralPath $sourceSound -PathType Leaf)){throw "No se encontro: $sourceSound"}

if($aiEnabled){Ensure-OllamaReady}

Write-Step 'Preparando directorios...'
@($installRoot,$moduleTarget,$uiTarget,$scriptsTarget,$assetsTarget,(Split-Path -Parent $configTarget),$sessionsPath,$exportsPath)|ForEach-Object{if(-not(Test-Path -LiteralPath $_)){New-Item -ItemType Directory -Path $_ -Force|Out-Null}}
...
$config=[ordered]@{
 ...
 brand=[ordered]@{name='PS-PowerPrompt';tagline='Entorno profesional de productividad para PowerShell';company='Puro Ingenio Samario'}
 agent=[ordered]@{name='PowerPrompt';mode='intelligent-local';message='Sesion protegida y lista para trabajar.';soundEnabled=$soundEnabled;startupSound=$startupSound}
 ai=[ordered]@{
  enabled=$aiEnabled;defaultProvider=$(if($aiEnabled){'Ollama'}else{'WindowsCopilot'})
  providers=[ordered]@{
   Ollama=[ordered]@{enabled=$aiEnabled;type='ollama';endpoint=$OllamaEndpoint;model=$OllamaModel;apiKeyEnvironmentVariable=''}
...
if($installContextMenu -and $pwsh){Write-Step 'Configurando menu contextual del Explorador...';Install-ContextMenu -PwshPath $pwsh -LauncherPath $launcherTarget}

Write-Step 'Validando modulo y comandos publicos...'
Remove-Module PSPowerPrompt -Force -ErrorAction SilentlyContinue
Import-Module $moduleManifest -Force
foreach($command in @('pp-start','pp-status','pp-new','pp-restart','pp-help','pp-export','pp-export-safe','pp-export-jsonl','pp-open','pp-stop','pp-panel','pp-set','pp-vars','pp-unset','pp-go','pp-ask','pp-explain','pp-fix','pp-ai-status','pp-ai-config','pp-update','pp-uninstall','pp-doctor','Show-PPCorporateWelcome')){if(-not(Get-Command $command -ErrorAction SilentlyContinue)){throw "No se pudo cargar el comando $command"}}

Write-Success 'Instalacion completada y validada.'
Write-Host "  Version: $InstallerVersion" -ForegroundColor DarkGray
Write-Host "  Asistencia IA: $aiEnabled $(if($aiEnabled){"(Ollama / $OllamaModel)"}else{'(desactivada)'})" -ForegroundColor DarkGray
Write-Host "  Configuracion: $configTarget" -ForegroundColor DarkGray; Write-Host ''

if(-not $NoLaunch -and $pwsh){
 Write-Step 'Abriendo PowerShell 7 configurado...'
 Start-Process -FilePath $pwsh -ArgumentList '-NoLogo'
 if($launchPanel){Start-Sleep -Milliseconds 800;$panel=Join-Path $uiTarget 'PowerPrompt.Panel.ps1';Start-Process -FilePath $pwsh -ArgumentList @('-NoLogo','-NoProfile','-STA','-ExecutionPolicy','Bypass','-File',"`"$panel`"")}
}