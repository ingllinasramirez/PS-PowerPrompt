...
function Add-Failure { param([string]$Message) $failures.Add($Message); Write-Host "[FAIL] $Message" -ForegroundColor Red }
...
$requiredFiles = @(
    ...
    'modules\PSPowerPrompt\PSPowerPrompt.Branding.psm1',
    'scripts\update.ps1',
...
foreach ($relativePath in $requiredFiles) {
    ...
    if (Test-Path -LiteralPath $fullPath) { Add-Success "Existe $relativePath" }
    else { Add-Failure "Falta $relativePath" }
}

$parseFiles = Get-ChildItem -LiteralPath $ProjectRoot -Recurse -File | Where-Object { $_.Extension -in @('.ps1','.psm1','.psd1') }
foreach ($file in $parseFiles) {
    $tokens = $null
    $errors = $null
    [void][System.Management.Automation.Language.Parser]::ParseFile($file.FullName, [ref]$tokens, [ref]$errors)
    if ($errors.Count -gt 0) {
        foreach ($errorItem in $errors) { Add-Failure "$($file.FullName): $($errorItem.Message)" }
    } else { Add-Success "Sintaxis valida: $($file.Name)" }
}
...
if (Test-Path -LiteralPath $manifestPath) {
    try {
        ...
        foreach ($command in $requiredCommands) {
            if (Get-Command $command -ErrorAction SilentlyContinue) { Add-Success "Comando disponible: $command" }
            else { Add-Failure "Comando no disponible: $command" }
        }
    }
    catch { Add-Failure "No fue posible validar/importar el modulo: $($_.Exception.Message)" }
    finally { Remove-Module PSPowerPrompt -Force -ErrorAction SilentlyContinue }
}
